...
import {
  ...
  apiGetContratistas, apiGetCronograma,
} from '../utils/api'
...
export function AppProvider({ children }) {
  ...
  const [data, setData]       = useState({ activos:[], sedes:[], mantenimientos:[], contratistas:[], cronograma:[] })
  const [loading, setLoading] = useState(true)

  // Carga todos los datos del servidor
  const reload = useCallback(async () => {
    try {
      const [activos, sedes, mantenimientos, contratistas, cronograma] = await Promise.all([
        apiGetActivos(),
        apiGetSedes(),
      ...
      setData({ activos, sedes, mantenimientos, contratistas, cronograma })
    } catch (e) {
